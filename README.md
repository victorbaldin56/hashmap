...
в геометрической прогрессии с множителем примерно $10^{1/5} \approx 1.58$ с расчетом
...
Построим график в логарифмическом масштабе по оси $x$:

![image](graphs/size_time.png)

Для лучшей визуализации в данных координатах проведена наилучшая экспоненциальная
кривая методом МНК. В исходных координатах эта кривая -- график степенной
функции. Численные методы библиотеки SciPy
дают оценку для показателя данной степенной функции:
$\alpha \approx 0.6$.
