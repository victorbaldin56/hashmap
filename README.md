...
кривая методом МНК. В исходных координатах эта кривая -- график степенной
функции.

**_ЗАМЕЧАНИЕ:_** аппроксимация степенной функцией использовалась исключительно
для лучшей визуализации. Разумеется, степенная функция неспособна корректно отражать
характер зависимости на еще более широком промежутке. Даже здесь эта модель,
на самом деле, не работает: видно, что половина точек лежит на кривой плохо.
Но если построить график в логарифмическом масштабе по обеим осям, можно заметить
некоторые любопытные свойства кривой:

![image](graphs/size_time_loglog.png)

Видно, что первые и следующие 8 точек по отдельности аппроксимируются прямой (то есть степенной
функцией в исходном масштабе)
с очень хорошей точностью. Случайно ли это совпадение? Полное исследование
зависимости мы в данной работе оставим за кадром, однако сложно привести
внятное теоретическое обоснование данного эффекта, кроме того, у нас на самом деле
слишком мало точек, чтобы мы могли сделать какой-либо вывод. Однако сам факт, конечно,
интересен.
...
Хеш-таблицы, например, очень хорошо оптимизируются увеличением размера,
к тому же в современных реалиях оперативная память не является большой проблемой.
